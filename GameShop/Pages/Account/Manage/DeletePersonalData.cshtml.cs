...
namespace GameShop.Areas.Identity.Pages.Account.Manage
{
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Не вдалося оновити користувача з таким ID '{userId}'.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("Користувач з таким ID '{userId}' видалив особисту інформацію", userId);
